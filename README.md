...
- [x] 6. Отсутствие возможности перезаписи файлов на ftp сервере в директории с ограниченным доступом
- [x] 7. Возможность удаления файлов с ftp сервера из директорий с ограниченным доступом.
...
- [x] 11. Соответствия названий файлов после скачивания.
- [x] 12. Соответствия размеров файлов после скачивания.
- [x] 13. Соответствие размера и времени при скачивании файла.
...
№ | Действие| Результат
-- | -- | --
1 | Зафиксировать размер и название  загружаемого файл|Данные зафиксированы
2 | Команда переход в директорию upload| Пользователь находится в директории /upload
3 | Проверка отсутствия (произвольного) загружаемого файла на сервере| Файла нет
4 | Команда загрузки файла на сервер |Получение от сервера сообщения  успешного завершения операции(загрузка завершена успешно)
...
1 |Проверка наличия файла на сервере | Файл есть
2 |Команда скачивания файла с сервера |  Получение от сервера сообщения  о успешном завершении операции (загрузка завершена)
3 |Проверка наличия загруженного файла на компьютере тестировщика | Скорость скачивания файла не ниже минимально допустимого предела
